  ...
  async ensureTrackingSubscriptions() {
    ...
    this._trackingPromise = (async () => {
      try {
        ...
        if (!messaging) {
          // Firebase is not initialized yet — skip Firebase subscriptions and do NOT
          // mark as subscribed so the next call can retry once Firebase is ready.
          console.warn('[PushOrchestrator] messaging unavailable, Firebase subscriptions skipped (will retry)')
          return false
        }
        ...
        if (messaging && !this._unsubOnNotificationOpened) {
          const unsub = this._deps.onNotificationOpenedApp(
            ...
            async (remoteMessage) => {
              ...
              await this._deps.updPushData(remoteMessage, shopId)
              await this._deps.pushClickListener(remoteMessage)
            }
          )
        ...
        if (messaging && !this._unsubOnTokenRefresh) {
          const unsub = this._deps.onTokenRefresh(messaging, (token) => {
            if (typeof token === 'string' && token.length > 0) {
              if (this._deps.isDebug()) console.log('[Firebase] Token refreshed:', token)
              this._deps.onNewToken(token)
            }
          })
          this._unsubOnTokenRefresh = typeof unsub === 'function' ? unsub : () => {}
        }
        ...
        if (!this._notifeeInitialChecked) {
          // FCM cold-start: app launched by tapping a system notification (app was killed)
          if (messaging) {
            const fcmInitial = await this._deps.getInitialNotification(messaging)
            if (fcmInitial) {
              const messageId = fcmInitial?.messageId
              if (!messageId || !this._deps.hasSeenMessageId(messageId)) {
                if (messageId) this._deps.markMessageIdSeen(messageId)
                if (this._deps.isDebug()) console.log('App cold-started via FCM notification', fcmInitial)
                await this._deps.updPushData(fcmInitial, shopId)
                await this._deps.pushClickListener(fcmInitial)
              }
            }
          }

          // Notifee cold-start: app launched by tapping a notifee-displayed notification
